...
- [ ] `play` — remove the decorative lines and keep the release inside the play shape.
- [ ] `search-01` — keep the handle joined to the lens throughout the sweep.
...
- [ ] `notification-off-01` — form a regular bell first, then draw the slash across it without clipping.
- [ ] `printer` — leave a visible output slot when the paper exits.
...
- [ ] `copy-01` — remove panel clipping during the copy handoff.
...
- [ ] `diamond-02` — enlarge the response and articulate the facets.
...
- [ ] `mail-open` — keep the letter and envelope edge continuous while the letter rises.
- [ ] `paint-brush-02` — replace the detached bottom line with a paint stroke from the brush.
- [ ] `pin` — remove internal line clipping during the pin action.
- [ ] `puzzle` — replace sparks with temporary neighboring puzzle pieces.
...
- [ ] `volume-mute-01` — keep the mute X clear of the speaker outline.

## People and groups

- [ ] `user-group` — show individual users gathering into a readable group.
- [ ] `user-multiple-02` — remove the overlap collision and bring two users together without merging outlines.

## Brand

- [ ] `github` — keep the body anchored while the tail carries a fluid base-to-tip swish and the ears answer with a subtle cat-like twitch.
